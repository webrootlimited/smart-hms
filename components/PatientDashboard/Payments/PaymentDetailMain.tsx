...
interface PaymentDetail {
  ...
  card: { card_type: string; last4: string; holder_name: string } | null;
  appointment: {
    id: string;
    type: string;
    scheduled_at: string;
...
export default function PaymentDetailMain() {
  const params = useParams();
  const paymentId = params.paymentId as string;
  const base = `/patient/${params.patientName}`;

  const { data, isLoading } = useQuery<{ success: boolean; payment: PaymentDetail }>({
    queryKey: ["patient", "payment", paymentId],
    queryFn: () => apiFetch(`/api/patient/payments/${paymentId}`),
    enabled: !!paymentId,
  });
  ...
  const color = payment.doctor ? getColor(payment.doctor.id) : "#6A7282";
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
        <p className="text-xs text-[#6A7282] uppercase tracking-wide">Amount Paid</p>
        <p className="text-3xl font-bold text-[#101828] mt-1">£{payment.amount}</p>
        {payment.paid_at && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {payment.doctor && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex items-center gap-3">
              {payment.doctor.photo_url ? (
                <img src={payment.doctor.photo_url} alt="" className="w-12 h-12 rounded-full object-cover shrink-0" />
              ) : (
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0"
                  style={{ backgroundColor: color }}
                >
                  {payment.doctor.initials}
                </div>
              )}
              <div>
                <p className="text-sm font-bold text-[#101828]">{payment.doctor.name}</p>
                <p className="text-xs text-[#0284C7]">{payment.doctor.specialty}</p>
                {payment.doctor.phone && (
                  <p className="text-xs text-[#6A7282] mt-0.5 flex items-center gap-1">
                    <Phone className="w-3 h-3" /> {payment.doctor.phone}
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="flex items-center gap-2">
                {appt.type === "PHYSICAL" ? (
                  <>
                    ...
                    <span className="text-sm text-[#101828]">In-Person</span>
                  </>
                ) : (
                  <>
                    <Video className="w-3.5 h-3.5 text-[#6A7282]" />
                    <span className="text-sm text-[#101828]">Online Consultation</span>
                  </>
                )}
              </div>
              <div className="flex items-center gap-2">
        ...
        {appt?.clinic && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            {appt.clinic.address && (
              <p className="text-xs text-[#6A7282] mt-1">{appt.clinic.address}</p>
            )}
            {appt.clinic.phone && (
            ...
            <div className="space-y-2">
              ...
              {payment.card.holder_name && (
                <p className="text-xs text-[#6A7282] flex items-center gap-1">
                  <User className="w-3 h-3" /> {payment.card.holder_name}
                </p>
              )}
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-lg font-bold text-[#101828]">£{payment.amount}</p>
