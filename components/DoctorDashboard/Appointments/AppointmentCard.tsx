...
const STATUS_STYLES: Record<string, { bg: string; color: string }> = {
  Scheduled: { bg: "bg-[#F0F9FF]", color: "text-[#0284C7]" },
  "In Progress": { bg: "bg-[#FFFBEB]", color: "text-[#D97706]" },
  Completed: { bg: "bg-[#F0FDF4]", color: "text-[#16A34A]" },
  Cancelled: { bg: "bg-[#FEF2F2]", color: "text-[#EF4444]" },
  "No Show": { bg: "bg-[#FAF5FF]", color: "text-[#7C3AED]" },
};
...
export default function AppointmentCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md transition">
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-[#E5E7EB] flex items-center justify-center shrink-0">
            <span className="text-xs font-bold text-[#4A5565]">{appointment.avatar}</span>
          </div>
          <div>
      ...
      {/* Type + Reason */}
      <div className="mb-3">
        <p className="text-xs font-semibold text-[#101828]">{appointment.type}</p>
        <p className="text-[11px] text-[#6A7282] mt-0.5 line-clamp-1">{appointment.reason}</p>
      </div>
      ...
      <div className="flex items-center gap-4 mb-3">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-[#4A5565]">{appointment.date}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Clock className="w-3 h-3 text-[#6A7282]" />
          <span className="text-[11px] text-[#4A5565]">{appointment.time}</span>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span className={`text-[11px] font-medium ${
            appointment.mode === "Telehealth" ? "text-[#0284C7]" : "text-[#16A34A]"
          }`}>
            {appointment.mode}
          </span>
        </div>
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
        ...
        {appointment.status === "Scheduled" && appointment.mode === "Telehealth" && (
          <Link
            href={detailHref}
